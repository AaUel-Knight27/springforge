...
public class SetupDbCommand implements Runnable {
    ...
    public void run() {
        try {
            ...
            for (Map.Entry<String, ServiceDefinition> entry : config.getServices().entrySet()) {
                ConsoleOutput.info("  • " + entry.getValue().getDatabaseName());
            }
            ConsoleOutput.newline();
